...
/**
* @param  {Array.<Annotation>} annotations consolidated annotation on a specific image
...
module.exports = function ({dist = defaultDist, threshold, annotations, scoreKey = 'confidence', mapFn = identity}) {
	const removeList = [];

	const mappedList = annotations.map(mapFn);

	const sortedList = mappedList
		.map((a, index) => Object.assign({}, a, {index}))
		.sort((a, b) => b[scoreKey] - a[scoreKey]);

	const matrix = sortedList.map(a => sortedList.map(b => {
